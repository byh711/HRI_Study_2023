...
audio_config = cf_ENG.audio_config

# set logger
logger2 = cf_ENG.logging.getLogger("__asr_ENG__")


def asr_tts_excel():
    if cf_ENG.scriptcounter == 1:
        with open(cf_ENG.script_path, encoding="utf-8-sig", newline='', mode="w") as f:
            writer = csv.writer(f)
            writer.writerow(['Speaker','Script','Start Time','Finish Time'])
            writer.writerow(cf_ENG.list_utter)
            cf_ENG.scriptcounter = cf_ENG.scriptcounter + 1
            
    elif cf_ENG.scriptcounter == 2:
        with open(cf_ENG.script_path, encoding="utf-8-sig", newline='', mode="a") as f:
            writer = csv.writer(f)
            writer.writerow(cf_ENG.list_utter)
...
class listen_micr:
    ...
    def run(self):
        ...
        speech_recognizer = speechsdk.SpeechRecognizer(speech_config=speech_config, language=cf_ENG.THIS_LANGUAGE, audio_config=audio_config)

        while self._running:
            try:
                result_future = speech_recognizer.recognize_once_async()
                logger2.debug('listening ...')
                # Inform a person to know start of recognition
                print("Say Something! - ENG")
                ...
                if user_utt:
                    ...
                    logger2.info('user said: ' + user_utt)
                    self.respond_to_user_utt(user_utt)

                    cf_ENG.list_utter.append("Player")
                    cf_ENG.list_utter.append(user_utt)
            ...
            # MS Azure recognizer errors are written below as exception
                ...
                logger2.info("No speech could be recognized: {}".format(result.no_match_details))
                pass
            except result.reason == speechsdk.ResultReason.Canceled:
                cancellation_details = result.cancellation_details
                logger2.info("Speech Recognition canceled: {}".format(cancellation_details.reason))
    ...
    def respond_to_user_utt(self, text):
        try:
            ...
            for keyword in cf_ENG.ASR_KEYS_UTTS.keys():
                ...
                if keyword in text :
                    ...
                    utterance = random.choice(ast.literal_eval(cf_ENG.ASR_KEYS_UTTS[keyword]))
        ...
        # If error occurs, write down the error at the logs/dm.log file
            ...
            logger2.error("user input parsing failed " + str(err))