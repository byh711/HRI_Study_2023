...
from Working_code_ENG import tts

# set logger
logger2 = cf_ENG.logging.getLogger("__parser__")
...
def get_utterance_from_abstract(abstract):
    ...
    if cf_ENG.priority_utterance_score[abstract] <= cf_ENG.CHATTINESS:
        # Evaluating safely the called utterance from the config file and get utterances as a list
        utt_list = ast.literal_eval(cf_ENG.RESPONSE_UTTS[abstract])
...
# Set up Repetition Delay. Based on the cf_ENG.REP_DELAY_AMT, If utterances print out third times, silent for 3 seconds
def repetition_delay(text):
    cf_ENG.LOCAL_REP_DELAY_AMT = cf_ENG.LOCAL_REP_DELAY_AMT - 1   # Reducing 1 'LOCAL_REP_DELAY_AMT' number

    cf_ENG.rep_delay_states[text] = cf_ENG.LOCAL_REP_DELAY_AMT    # Change 'rep_delay_states' to current 'LOCAL_REP_DELAY_AMT'

    # if 0, silent for 3 seconds
    if cf_ENG.rep_delay_states[text] == 0:
        time.sleep(3)

    # If 'LOCAL_REP_DELAY_AMT' is 0, reset the number using 'REP_DELAY_AMT'
    if cf_ENG.LOCAL_REP_DELAY_AMT == 0:
        cf_ENG.LOCAL_REP_DELAY_AMT = cf_ENG.REP_DELAY_AMT
...
def parse_day_subtree(current_state, initial_state):
    ...
    if current_state['Phase'] == 'day':
        cf_ENG.status['Phase'].append(current_state['Phase'])
        ...
        if len(cf_ENG.status['Phase']) == 1:
            tts.synthesize_utt(utterance)

        # If it is not the first one, just speak when there's difference occurs
        elif len(cf_ENG.status['Phase']) > 1:
            if cf_ENG.status['Phase'][-2] != cf_ENG.status['Phase'][-1]:
                tts.synthesize_utt(utterance)
    ...
    # Hunger starts
    # Hunger - very low
    if int(float(current_state['Hunger_AVATAR'])) < cf_ENG.STARVING:
        cf_ENG.status['Hunger_AVATAR'].append(int(float(current_state['Hunger_AVATAR'])))

        action = 'hunger_is_dangerous'
        utterance = get_utterance_from_abstract('inform_starving')

        # If it is the first one, speak
        if len(cf_ENG.status['Hunger_AVATAR']) == 1:
            tts.synthesize_utt(utterance)

        # If it is not the first one, just speak like below situation
        elif len(cf_ENG.status['Hunger_AVATAR']) > 1:
            # When there's difference occurs and 'Hunger' status is bigger than 25, then speak
            if cf_ENG.status['Hunger_AVATAR'][-2] < cf_ENG.status['Hunger_AVATAR'][-1] and cf_ENG.status['Hunger_AVATAR'][-1] > 25:
                tts.synthesize_utt(utterance)
            ...
            elif cf_ENG.status['Hunger_AVATAR'][-2] == 29 and cf_ENG.status['Hunger_AVATAR'][-1] != 29:
                tts.synthesize_utt(utterance)

    # Hunger - low
    elif cf_ENG.HUNGRY >= int(float(current_state['Hunger_AVATAR'])) > cf_ENG.STARVING:
        cf_ENG.status['Hunger_AVATAR'].append(int(float(current_state['Hunger_AVATAR'])))
        ...
        # If it is not the first one and if the Hunger status is 31 ~ 45, speak when there's difference occurs
            ...
            if cf_ENG.status['Hunger_AVATAR'][-2] < cf_ENG.status['Hunger_AVATAR'][-1] and cf_ENG.status['Hunger_AVATAR'][-1] > 45:
                tts.synthesize_utt(utterance)
    # Hunger ends
    ###############################################################################################
    ...
    # Health starts
    # Health - very low
    if int(float(current_state['Health_AVATAR'])) < cf_ENG.DYING:
        cf_ENG.status['Health_AVATAR'].append(int(float(current_state['Health_AVATAR'])))

        action = 'Almost_Dying'
        utterance = get_utterance_from_abstract('inform_dying')  # en

        # If it is the first one, speak
        if len(cf_ENG.status['Health_AVATAR']) == 1:
            tts.synthesize_utt(utterance)

        # If it is not the first one, just speak like below situation
        elif len(cf_ENG.status['Health_AVATAR']) > 1:
            # When there's difference occurs and 'Health' status is bigger than 25, then speak
            if cf_ENG.status['Health_AVATAR'][-2] < cf_ENG.status['Health_AVATAR'][-1] and cf_ENG.status['Health_AVATAR'][-1] > 25:
                tts.synthesize_utt(utterance)

            # If the Health status is 29, to reduce a bit of repetition (30->29->30->29),
            # Only speaks when the just before the latest one is 29
            elif cf_ENG.status['Health_AVATAR'][-2] == 29 and cf_ENG.status['Health_AVATAR'][-1] != 29:
                tts.synthesize_utt(utterance)

    # Health - low
    elif cf_ENG.INJURED > int(float(current_state['Health_AVATAR'])) > cf_ENG.DYING:
        cf_ENG.status['Health'].append(int(float(current_state['Health_AVATAR'])))
        ...
        # If it is not the first one and if the Health status is 31 ~ 45, speak when there's difference occurs
            ...
            if cf_ENG.status['Health_AVATAR'][-2] < cf_ENG.status['Health_AVATAR'][-1] and cf_ENG.status['Health_AVATAR'][-1] > 45:
                tts.synthesize_utt(utterance)
    # Health ends
    ###############################################################################################
    ...
    # Sanity starts
    # Sanity - Very low
    if int(float(current_state['Sanity_AVATAR'])) < cf_ENG.SANITY_DANGER:
        cf_ENG.status['Sanity_AVATAR'].append(int(float(current_state['Sanity_AVATAR'])))

        action = 'Mental_Crushed'
        utterance = get_utterance_from_abstract('inform_getting_insane') # en # en

        # If it is the first one, speak
        if len(cf_ENG.status['Sanity_AVATAR']) == 1:
            tts.synthesize_utt(utterance)

        # If it is not the first one, just speak like below situation
        elif len(cf_ENG.status['Sanity_AVATAR']) > 1:
            # When there's difference occurs and 'Sanity' status is bigger than 25, then speak
            if cf_ENG.status['Sanity_AVATAR'][-2] < cf_ENG.status['Sanity_AVATAR'][-1] and cf_ENG.status['Sanity_AVATAR'][-1] > 25:
                tts.synthesize_utt(utterance)

            # If the Sanity status is 29, to reduce a bit of repetition (30->29->30->29),
            # Only speaks when the just before the latest one is 29
            elif cf_ENG.status['Sanity_AVATAR'][-2] == 29 and cf_ENG.status['Sanity_AVATAR'][-1] != 29:
                tts.synthesize_utt(utterance)

    # Sanity - low
    elif 50 > int(float(current_state['Sanity_AVATAR'])) > cf_ENG.SANITY_DANGER :
        cf_ENG.status['Sanity_AVATAR'].append(int(float(current_state['Sanity_AVATAR'])))
        ...
        # If it is not the first one and if the Sanity status is 31 ~ 45, speak when there's difference occurs
            ...
            if cf_ENG.status['Sanity_AVATAR'][-2] < cf_ENG.status['Sanity_AVATAR'][-1] and cf_ENG.status['Sanity_AVATAR'][-1] > 45:
                tts.synthesize_utt(utterance)
    # Sanity ends
    ###############################################################################################
    ...
    if current_state['Curr_Equip_Hands_AVATAR'] != 'nil':
        ...
        # If it is the first one, speak
        if len(cf_ENG.status['Curr_Equip_Hands_AVATAR']) == 1:
            tts.synthesize_utt(utterance)

        # If it is not the first one, just speak when there's difference occurs
        elif len(cf_ENG.status['Curr_Equip_Hands_AVATAR']) > 1:
            if cf_ENG.status['Curr_Equip_Hands_AVATAR'][-2] != cf_ENG.status['Curr_Equip_Hands_AVATAR'][-1]:
                ...
                if cf_ENG.rep_delay_states['Curr_Equip_Hands_AVATAR'] != 0:
                    tts.synthesize_utt(utterance)
    # Equipment ends
    ###############################################################################################
    ...
    if current_state['Attack_Target_AVATAR'] != 'nil':
        ...
        # If it is the first one, speak
        if len(cf_ENG.status['Attack_Target_AVATAR']) == 1:
            tts.synthesize_utt(utterance)

        # If it is not the first one, just speak when there's difference occurs
        elif len(cf_ENG.status['Attack_Target_AVATAR']) > 1:
            if cf_ENG.status['Attack_Target_AVATAR'][-2] != cf_ENG.status['Attack_Target_AVATAR'][-1]:
                # Apply repetition delay function for current category (The function starts at line 28)
                repetition_delay('Attack_Target_AVATAR')

                # If the rep_delay_states number(count) is not 0, then utterances keep coming out
                if cf_ENG.rep_delay_states['Attack_Target_AVATAR'] != 0:
                    tts.synthesize_utt(utterance)
    # Attack ends
    ###############################################################################################
    ...
    if current_state['Defense_Target_AVATAR'] != 'nil':
        ...
        # If it is the first one, speak
        if len(cf_ENG.status['Defense_Target_AVATAR']) == 1:
            tts.synthesize_utt(utterance)

        # If it is not the first one, just speak when there's difference occurs
        elif len(cf_ENG.status['Defense_Target_AVATAR']) > 1:
            if cf_ENG.status['Defense_Target_AVATAR'][-2] != cf_ENG.status['Defense_Target_AVATAR'][-1]:
                # Apply repetition delay function for current category (The function starts at line 28)
                repetition_delay('Defense_Target_AVATAR')

                # If the rep_delay_states number(count) is not 0, then utterances keep coming out
                if cf_ENG.rep_delay_states['Defense_Target_AVATAR'] != 0:
                    tts.synthesize_utt(utterance)
    # Defense ends
    ###############################################################################################
    ...
    #     cf_ENG.status['Food_AVATAR'].append(current_state['Food_AVATAR'])
    #
    #     action = 'Food_is_needed_immediately'
    #     utterance = get_utterance_from_abstract('inform_no_food')
    #     # utterance = "We have no food now." # en
    #
    #     if len(cf_ENG.status['Food_AVATAR']) == 1:
    #         tts.synthesize_utt(utterance)
    #     elif len(cf_ENG.status['Food_AVATAR']) > 1:
    #         if cf_ENG.status['Food_AVATAR'][-2] != cf_ENG.status['Food_AVATAR'][-1]:
    #             tts.synthesize_utt(utterance)
    ...
    if current_state['Food_AVATAR'] == 'Less Food!':
        ...
        # If it is not the first one, just speak when there's difference occurs
            ...
            if cf_ENG.status['Food_AVATAR'][-2] != cf_ENG.status['Food_AVATAR'][-1]:
                ...
                # If the rep_delay_states number(count) is not 0, then utterances keep coming out
                if cf_ENG.rep_delay_states['Food_AVATAR'] != 0:
                    tts.synthesize_utt(utterance)
    ...
    #     cf_ENG.status['Tool_AVATAR'].append(current_state['Tool_AVATAR'])
    #
    ...
    #
    #     if len(cf_ENG.status['Tool_AVATAR']) == 1:
    #         tts.synthesize_utt(utterance)
    #     elif len(cf_ENG.status['Tool_AVATAR']) > 1:
    #         if cf_ENG.status['Tool_AVATAR'][-2] != cf_ENG.status['Tool_AVATAR'][-1]:
    #             tts.synthesize_utt(utterance)
    ...
    if current_state['Tool_AVATAR'] == 'Only Axe' :
        ...
        # If it is not the first one, just speak when there's difference occurs
            ...
            if cf_ENG.status['Tool_AVATAR'][-2] != cf_ENG.status['Tool_AVATAR'][-1]:
                ...
                # If the rep_delay_states number(count) is not 0, then utterances keep coming out
                if cf_ENG.rep_delay_states['Tool_AVATAR'] != 0:
                    tts.synthesize_utt(utterance)
    ...
    #     cf_ENG.status['Lights_AVATAR'].append(current_state['Lights_AVATAR'])
    #     cf_ENG.status['Is_Light_AVATAR'].append(current_state['Is_Light_AVATAR'])
    #
    ...
    #
    #     if len(cf_ENG.status['Lights_AVATAR']) == 1:
    #         tts.synthesize_utt(utterance)
    #     elif len(cf_ENG.status['Lights_AVATAR']) > 1:
    #         if cf_ENG.status['Lights_AVATAR'][-2] != cf_ENG.status['Lights_AVATAR'][-1]:
    #             tts.synthesize_utt(utterance)
    ...
    if current_state['Lights_AVATAR'] == 'Only torch' and current_state['Is_Light_AVATAR'] == 'No lights!' :
        ...
        # If it is not the first one, just speak when there's difference occurs
            ...
            if cf_ENG.status['Lights_AVATAR'][-2] != cf_ENG.status['Lights_AVATAR'][-1]:
                ...
                # If the rep_delay_states number(count) is not 0, then utterances keep coming out
                if cf_ENG.rep_delay_states['Lights_AVATAR'] != 0:
                    tts.synthesize_utt(utterance)
    ...
    if current_state['Is_Light_AVATAR'] == 'Lights nearby' and str(current_state['Curr_Equip_Hands_AVATAR'][9:]) != 'torch(LIMBO)':
        ...
        # If it is the first one, speak
        if len(cf_ENG.status['Is_Light_AVATAR']) == 1:
            tts.synthesize_utt(utterance)

        # If it is not the first one, just speak when there's difference occurs
        elif len(cf_ENG.status['Is_Light_AVATAR']) > 1:
            if cf_ENG.status['Is_Light_AVATAR'][-2] != cf_ENG.status['Is_Light_AVATAR'][-1]:
                # Apply repetition delay function for current category (The function starts at line 28)
                repetition_delay('Is_Light_AVATAR')

                # If the rep_delay_states number(count) is not 0, then utterances keep coming out
                if cf_ENG.rep_delay_states['Is_Light_AVATAR'] != 0:
                    tts.synthesize_utt(utterance)
    ...
    if current_state['Is_Monster_AVATAR'] == 'Too many monsters':
        cf_ENG.status['Is_Monster_AVATAR'].append(current_state['Is_Monster_AVATAR'])

        action = 'monsters_are_lot_nearby'
        utterance = get_utterance_from_abstract('inform_lots_of_monsters')

        # If it is the first one, speak
        if len(cf_ENG.status['Is_Monster_AVATAR']) == 1:
            tts.synthesize_utt(utterance)

        # If it is not the first one, just speak when there's difference occurs
        elif len(cf_ENG.status['Is_Monster_AVATAR']) > 1:
            if cf_ENG.status['Is_Monster_AVATAR'][-2] != cf_ENG.status['Is_Monster_AVATAR'][-1]:
                # Apply repetition delay function for current category (The function starts at line 28)
                repetition_delay('Is_Monster_AVATAR')

                # If the rep_delay_states number(count) is not 0, then utterances keep coming out
                if cf_ENG.rep_delay_states['Is_Monster_AVATAR'] != 0:
                    tts.synthesize_utt(utterance)
    ...
    if current_state['Player_Xloc'] != '999':
        cf_ENG.status['Player_Xloc'].append(float(current_state['Player_Xloc']))
        
        # At night, the movement is limited, so It works only for 'day' and 'dusk' phase
        if cf_ENG.status['Phase'][-1] == 'day' or cf_ENG.status['Phase'][-1] == 'dusk':
            # if player's location is same for 25 rows, then speak utterance
            if len(cf_ENG.status['Player_Xloc']) >= 40:
                if cf_ENG.status['Player_Xloc'][-1] == cf_ENG.status['Player_Xloc'][-40]:
